...
interface UseVideoCallReturn {
  ...
  toggleMedia: (matchId: string, mediaType: 'audio' | 'video', enabled: boolean) => void
}
...
export const useVideoCall = (options: UseVideoCallOptions = {}): UseVideoCallReturn => {
  ...
  useEffect(() => {
    ...
    const handleCallRinging = (data: { matchId: string; calleeId: string; channelName: string }) => {
    ...
    const handleCallAccepted = (data: { matchId: string; channelName: string; calleeId: string }) => {
      setCallStatus('connected')
      setCurrentCall(prev => prev ? { ...prev, channelName: data.channelName } : null)
      options.onCallAccepted?.(data)
    }

    // Call connected (for callee)
    const handleCallConnected = (data: { matchId: string; channelName: string; callerId: string }) => {
      setCallStatus('connected')
      setCurrentCall({
        matchId: data.matchId,
        channelName: data.channelName,
    ...
    // Call declined
    const handleCallDeclined = (data: { matchId: string; calleeId: string }) => {
      setCallStatus('ended')
      setCurrentCall(null)
    ...
    // Call ended
    const handleCallEnded = (data: { matchId: string; endedBy: string; duration?: number }) => {
      setCallStatus('ended')
      setCurrentCall(null)
      setIncomingCall(null)
    ...
    // Call cancelled
    const handleCallCancelled = (data: { matchId: string; callerId: string }) => {
      setCallStatus('idle')
      setIncomingCall(null)
  ...
  // Initiate a call
  const initiateCall = useCallback((matchId: string, calleeId: string) => {
  ...
  const cancelCall = useCallback((matchId: string) => {
    if (socket && isConnected) {
  ...
  const toggleMedia = useCallback((matchId: string, mediaType: 'audio' | 'video', enabled: boolean) => {
