...
export const useStagedCallHandlers = ({
  ...
  const handleRinging = useCallback(
    (data: StagedCallRingingPayload) => {
      const isActuallyBusy = user?.id
        ? busyStates[user.id] === "in-call"
        : false;
  ...
  const handleConnected = useCallback(
    (data: StagedCallAcceptedPayload) => {
  ...
  const handleContactExchange = useCallback(
    (data: ContactExchangePayload) => {
