...
import type { AgoraJoinOptions, AgoraJoinResult, AgoraEventHandlers } from './types'
...
export async function performLeave(
    ...
    try {
        const tracksToUnpublish = [localAudioTrack, localVideoTrack].filter(Boolean) as (IMicrophoneAudioTrack | ICameraVideoTrack)[]

        if (tracksToUnpublish.length > 0 && client && client.connectionState === 'CONNECTED') {
            try {
