 ...
 */
export const createAudioTrackWithRetry = async (
  maxRetries = 3,
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.warn(
        `[Agora] Creating audio track (attempt ${attempt}/${maxRetries})...`,
      );
      ...
      console.warn(
        `[Agora] Audio track creation failed (attempt ${attempt}):`,
        error,
      );
 ...
 */
export const createVideoTrackWithRetry = async (
  maxRetries = 2,
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.warn(
        `[Agora] Creating video track (attempt ${attempt}/${maxRetries})...`,
      );
      ...
      lastError = error as Error;
      console.warn(
        `[Agora] Video track creation failed (attempt ${attempt}):`,
        error,
      );
