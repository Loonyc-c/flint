...
export class AgoraClient {
  ...
  private async executeJoin(
    ...
    try {
      ...
      if (
        ...
        timeSinceDisposal < this.MIN_TRACK_RECREATION_DELAY
      ) {
        await new Promise((resolve) =>
          setTimeout(
            resolve,
            this.MIN_TRACK_RECREATION_DELAY - timeSinceDisposal,
          ),
        );
      }
  ...
  async disableVideo(): Promise<void> {
    this.localVideoTrack = await disableVideo(
      this.client,
      this.localVideoTrack,
    );
  }
...
export const agoraClient =
  typeof window !== "undefined" ? new AgoraClient() : ({} as AgoraClient);
